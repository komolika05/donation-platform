...
router.get("/dashboard", authenticate, authorize("donor"), async (req: AuthRequest, res) => {
  try {
    ...
    const assignedCases = await CaseReport.find({ donor: userId }).sort({ createdAt: -1 })
    ...
    const balance = (totalDonated[0]?.total || 0) - (totalCaseCosts[0]?.total || 0)
...
router.get("/admin/dashboard", authenticate, authorize("super-admin"), async (req: AuthRequest, res) => {
  try {
    logger.info("Admin dashboard data requested", { userId: req.user!._id })
...
router.get("/hospital-admin/dashboard", authenticate, authorize("hospital-admin"), async (req: AuthRequest, res) => {
  try {
    const userId = req.user!._id

    logger.info("Hospital admin dashboard data requested", { userId })

    // Get reports uploaded by this hospital admin
    const myReports = await CaseReport.find({ uploadedBy: userId }).sort({ createdAt: -1 })
    ...
    logger.info("Hospital admin dashboard data retrieved successfully", {
      userId,
    ...
    logger.error("Hospital admin dashboard data fetch error:", error)
    res.status(500).json({
      success: false,
